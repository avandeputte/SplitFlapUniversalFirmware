...
9. **Motor functions** — `applyStep`, `advance` (the single forward half-step primitive, formerly `stepBackward`), `releaseMotor`, `hallActive`, `nudge`, `homeModule`, `calibrateModule`.
...
### Surviving a busy module

Bytes are received by SoftwareSerial's interrupt into a 128-byte ring (raised from the 64-byte default via `-D_SS_MAX_RX_BUFF=128` in `platformio.ini`, so every `N` flap-set frame fits even when the module cannot drain it). While the firmware is inside a multi-second move, calibration, or EEPROM burst it does not drain the ring, and anything beyond its capacity is dropped. The danger is not the lost bytes but the *splice*: the head of the cut frame followed by the tail of a later one, which a naive parser would happily commit to EEPROM as a truncated character set or flap map. `loop()` therefore checks SoftwareSerial's overflow flag before every byte it reads (the blocking work is usually the handler of the byte just parsed, so a once-per-pass check would miss it). If the flag is set, the ring is still full and everything in it is from before the gap — complete frames first, then the head of the one frame that was cut — so those bytes are parsed normally, but once the last of them is consumed any frame still open is abandoned (`resetParser()`) and everything up to the next terminator is discarded. A reply's post-transmit drain (`txEnd`) applies the same rule if it finds the flag set, since the drain has just thrown away the cut frame's head. This also fixes a display-level symptom on large walls: a full-wall update streams more bytes than the ring holds while the first modules are already moving, and the splice could read as a valid frame for the wrong module (`m1` from one frame plus `3-C` from another). A catch-all 200 ms idle timeout also resets *any* parser state, so a lone `m` from bus noise or a lost terminator can never leave the parser waiting to swallow the next frame's bytes into the current field.

Two other things are gated on the parser being idle (`parseState == 0`): the deferred broadcast reply, which would otherwise transmit on top of a frame already on the bus and — for an `A` reply — overwrite the shared work buffer under an incoming restore; and the unprovisioned-module advertisement, which used to flush the receive buffer before its carrier-sense check and could eat the middle of a restore addressed to that very board.
...
EEPROM writes go through an `update`-style helper that reads first and only writes changed bytes, to spare the limited write-endurance of the cells. The helper also pulses the watchdog on every call: a full restore or factory reset on a fully calibrated module is roughly 300 back-to-back byte writes, close enough to the 2 s watchdog that a reset mid-restore (map cleared, half rewritten) was a real risk. A small write-read-verify check (using the scratch byte) backs the `eepromOk` field in the `Q` diagnostics; it costs two write cycles on that cell, so it runs on the first `Q` after boot and then at most hourly, with the cached result reported in between — a controller can poll `Q` freely without wearing the cell out.

Two details of the boot path protect against half-written state. On a blank or unrecognised chip the magic byte is written *last*, after every default field, so a power loss midway leaves "not initialised" rather than a valid magic over `0xFF` fields (which would, among other things, read as auto-home disabled). And the saved step position at `0x07` is read and written as the 2-byte field the layout defines; `currentStepPos` itself is a `long`, and writing it directly used to spill over the saved index and boot counter, then read those bytes back into the position so the restore-on-boot clamp rejected it every time. Calibration scalars loaded at boot or supplied by a restore go through one shared sanity clamp, so a garbled payload can never persist a zero-length revolution.

> **Brown-out detection:** the fuses enable BOD at 2.6 V in active mode. At 10 MHz the part needs 2.7 V; with BOD disabled it starts executing at the ~1.6 V power-on-reset threshold and keeps running down to it on power-off, and in that band flash and EEPROM reads are unreliable and a wild jump can trigger an EEPROM write. Every AVR datasheet names this as the cause of EEPROM corruption and BOD as the fix. Holding the chip in reset until the rail is good is what makes the boot-time magic-byte read trustworthy.
...
All movement is forward-only, in half-steps, through `advance(n)` — the single motion primitive. Each step pulses the watchdog, walks the half-step phase, and tracks the absolute step position, wrapping at one revolution. A Hall transition seen mid-move re-synchronises the absolute position to the known home location.
